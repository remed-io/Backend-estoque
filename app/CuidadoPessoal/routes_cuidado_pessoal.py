...
from app.CuidadoPessoal.schema_cuidado_pessoal import CuidadoPessoalCreate, CuidadoPessoalRead, CuidadoPessoalWithSubcategoria
...
@router.get("/{id}", response_model=CuidadoPessoalWithSubcategoria)
def get_cuidado_pessoal_by_id(id: int, db: Session = Depends(get_db)):
    # Busca cuidado pessoal e subcategoria associada para exibir nome e descrição da subcategoria
    ...
    sub = None
    if cuidado_pessoal.subcategoria_id:
        sub = db.query(SubcategoriaCuidadoPessoal).filter(
            SubcategoriaCuidadoPessoal.id == cuidado_pessoal.subcategoria_id
        ).first()
    return CuidadoPessoalWithSubcategoria(
        **cuidado_pessoal.__dict__,
        subcategoria_nome=sub.nome if sub else None,
        subcategoria_descricao=sub.descricao if sub else None
    )
